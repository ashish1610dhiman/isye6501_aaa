...
From the aove plot we can observe that there seems to be a linear relationship between the dependent variable and one of the highly correlated feature. Thus our assumption of linearity holds true and we can implement linear regression model.
...
* The predicted crime_rate for the given data point is `r crime_rate_pred`. Since this prediction lies outside the range of or dependent variable, we can further tune the model as the features of the predicting data point lie withing the domain of independent variables. \

* From the 4 diagnostic plots associated with the linear regression model, we can see that the assumptions are mostly in place, error terms are normally distributed i.e. constant variance assumption and residuals are homogeneous around the fitted line. However , we can see slight bends in the left and right end in the plot of residuals vs fitted values. Thus, a possible remedy can be thought of as transforming the dependent variable (taking a log transformation reduces the range of possible variables).

* Further from the QQ Plot it can be inferred that the data is normally distributed. Thus our normality assumption also holds true.
...
* As we can observe that after removing the potential outliers, there is no change in the model. Thus we can conclude that our assumption to classify the removed points as outliers is wrong.
