...
* From the 4 diagnostic plots associated with the linear regression model , we can see that the assumptions are mostly in place , error terms are normally distributed and residuals are homogeneous around the fitted line. However , we can see slight bends in the left and right end in the plot of residuals vs fitted values. Thus , a possible remedy can be thought of as modelling the log of dependent variable based on independent variable. (because taking a log transformation reduces the range of possible variables.)
