...
df_copy$split = "train"
df_copy$split[70 < df_copy2$split_i] = "valid"
df_copy$split[85 < df_copy2$split_i] = "test"

summary_df = aggregate(df_copy,list(df_copy$split), 
...
```{r}
k_optimal = acc_df2[which.max(acc_df$mean_cv_accuracy),1]
...
#### Run the train valid func function for variety of k values
...
\* From the above graph, we can see that increasing the value of 'K' beyond 4 yields very low marginal benefit. \* Both 3 and 4 appear to be good values of 'K' and can be used depending on the use case. In this example, since we know that there are 3 classes, we expect 3 to be a more appropriate choice.
...
From the confusion matrix, we can infer the following: \* Cluster 3 shows highest proportion of setosa class. In fact it encompasses 100% of that class. \* Cluster 2 has the highest proportion of versicolor class. \* Cluster 1 has the highest proportion of virginia class.
