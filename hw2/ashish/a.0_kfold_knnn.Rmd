...
acc_df <- data.frame(matrix(ncol = 0, nrow = length(seq(3,101,2))))
acc_df$k_kknn = seq(3,101,2)
mean_cv_acc_k = lapply(seq(3,101,2), function(x) mean(unlist(cv_function(df = df_copy,x))))
min_cv_acc_k = lapply(seq(3,101,2), function(x) min(unlist(cv_function(df = df_copy,x))))
